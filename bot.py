...
def select_image(title="Select an Image"):
    file_path = filedialog.askopenfilename(
        title=title,
        filetypes=[("Image Files", "*.png;*.jpg;*.jpeg;*.JPG;*.JPEG;*.PNG")]
    )
...
def create_wallpaper_grid():
    ...
    rows, cols = 5, 3  # Change as needed
    ...
    remaining_images_paths = filedialog.askopenfilenames(
        title="Select Remaining Images",
        filetypes=[("Image Files", "*.png;*.jpg;*.jpeg;*.JPG;*.JPEG;*.PNG")]
    )
    ...
    for img_path in remaining_images_paths:
        cropped_img = crop_to_square(img_path)
        if cropped_img:
